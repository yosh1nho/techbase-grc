...
(() => {
  ...
  `).join("");
    ...
    function openViewerModal(doc) {
        ...
        // Botão IA — sempre visível, abre modal de IA (ponto 2 — placeholder)
        ...
        // Botão de nova versão no viewer
        ...
        // Painel de sugestões — reset enquanto carrega
        ...
        // Disparar análise automática se o documento tem ficheiro analisável
        const analysableMimes = ["application/pdf", "text/plain", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"];
        const analysableName = /\.(pdf|txt|md|docx)$/i;
        const canAnalyse = doc.has_file && (
            analysableMimes.includes((doc.mime_type || "").toLowerCase()) ||
            analysableName.test(doc.file_name || "")
        );
        if (canAnalyse && sugPanel && sugBody) {
            sugPanel.style.display = "block";
            sugBody.innerHTML = '<div class="muted" style="font-size:12px;padding:8px 0">A analisar controlos cobertos...</div>';
            loadDocumentSuggestions(doc.id, sugBody);
        }
    ...
    function openAiAssistModal(doc) {
        ...
        if (!m) return;
        // doc pode ser null quando chamado do botão de upload
        ...
        if (out) out.value = "";  // textarea usa .value
        m.style.display = "";
        m.classList.add("open");
        m.setAttribute("aria-hidden", "false");
        document.body.style.overflow = "hidden";
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById("aiSaveAsDoc")?.addEventListener("click", () => {
            const out = document.getElementById("aiOutput");
            const text = out?.value?.trim() || "";
            if (!text) { alert("Gera primeiro um texto com a IA."); return; }
            ...
            closeAiAssistModal();
            showFeedback("ok", "Texto da IA carregado como ficheiro. Revê e clica em 'Fazer upload'.");
        });
    ...
    async function loadDocumentSuggestions(docId, container) {
        try {
            ...
            if (!res.ok || !data.success) {
                container.innerHTML = `<div class="muted" style="font-size:12px">${data.message || "Sem sugestões disponíveis."}</div>`;
                return;
            }
            renderSuggestions(data.suggestions || [], container);
        } catch (e) {
            container.innerHTML = `<div class="muted" style="font-size:12px">Erro ao analisar: ${e.message}</div>`;
        }
    }

    function renderSuggestions(suggestions, container) {
        if (!suggestions.length) {
            container.innerHTML = '<div class="muted" style="font-size:12px">Nenhum controlo identificado com confiança suficiente.</div>';
            return;
        }
        const coverageConfig = {
            high: { cls: "ok", label: "Alta", color: "#34d399" },
            medium: { cls: "warn", label: "Média", color: "#fbbf24" },
            low: { cls: "", label: "Baixa", color: "#94a3b8" },
        };
        container.innerHTML = `
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        ...
        <tbody>
          ${suggestions.map(s => {
            const cfg = coverageConfig[s.coverage] || coverageConfig.low;
            ...
            return `<tr>
              ...
              <td style="padding:10px 8px;border-bottom:1px solid rgba(255,255,255,.04);vertical-align:top;white-space:nowrap">
                <span class="tag ${cfg.cls}" style="font-size:11px"><span class="s"></span> ${s.score.toFixed(2)}</span>
                <div style="font-size:10px;color:var(--muted);margin-top:3px">${cfg.label}</div>
              </td>
              <td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,.04);vertical-align:top;color:var(--muted);font-size:12px">${just}</td>
            </tr>`;
        }).join("")}
        </tbody>
      </table>`;
